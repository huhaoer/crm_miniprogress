...
Page({
  ...
  gotoAddPlan(e) {
    ...
    wx.navigateTo({
      url: '/pages/addVisitRecord/addVisitRecord?proid=' + proid + '&planData=' + JSON.stringify(planData)//传递数据到 要提交计划
    });
  },
  // 3.生命周期  ===  请求下面拜访记录列表数据
  getVisitList() {
    const that = this;
    cloudFunc('getProjectDetail',{
      ...
      .then(res => {
          ...
          result.visitListPlan.forEach(item => {
            item.VisitPlanTime = item.VisitPlanTime && item.VisitPlanTime.split('T')[0].replace(/\-/g,".");//时间处理
          })
          // 已经拜访列表  处理时间戳 转换为正常时间
          result.aloneVisit.forEach(item => {
            item.VisitFactTime = item.VisitFactTime && item.VisitFactTime.split('T')[0].replace(/\-/g,".");//时间处理
            item.VisitPlanTime = item.VisitPlanTime && item.VisitPlanTime.split('T')[0].replace(/\-/g,".");//时间处理
          })
          // 重新赋值渲染的数据
          console.log(result.visitListPlan,result.aloneVisit,'处理过后的数据  时间')
          that.setData({
